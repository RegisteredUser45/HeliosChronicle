...
pub use contact::{
    ...
    sense_fleet, lose_fleet_contact, share_fleet_fog, sign_contract, sign_treaty, upgrade_fog, Contract, ContractKind, EmpireContact,
...
pub use world::World;
pub use worlds::{apply_facility_soaks, apply_layer_burst, apply_life_support_drain, apply_pop_deficits, compute_deficits, consume_stock_at_system, evacuate_body, facility_bom_recipe, install_facility_on_body, life_support_bill, reaggregate_after_life_support_drain, remediate_body_layers, remediation_stock_cost, DeficitReport, WorldsError};
pub use sensors::{
...
pub use hulls::{apply_ship_damage, apply_ship_maintenance, apply_ship_maintenance_on_world, build_ship, build_ship_at_system, can_fire, crew_ok, cargo_capacity, can_sense, can_move, design_has_module, derive_fuel_tier, fire_kinetic, jettison_cargo, fuel_recipe_for_tier, load_cargo, unload_cargo, load_magazine, maintenance_burn_rate, spend_magazine, make_design, refine_fuel_at_system, refine_ship_tier_fuel, refuel, refuel_from_system, register_design, restock_magazine_from_system, spend_fuel, spawn_instance, tool_yard, try_move, try_move_ship, transfer_cargo_ships, fire_kinetic_ship, load_cargo_on_ship, yard_build_ship, commission_design_at_system, auto_tool_design_if_yard_ready, transfer_cargo, transfer_fuel, tool_yard_at_system, unlock_design_from_completed_segment, HullError, ShipDesign, ShipInstance};
...
mod tests {
    ...
    fn hot_ttl_cools_stamped_system() {
        ...
        assert_eq!(w.ledger().get(id).unwrap().lod_hint, LodHint::Hot);
        w.tick(2);
        assert_eq!(w.ledger().get(id).unwrap().lod_hint, LodHint::Quiet);
        assert_eq!(w.ledger().get(id).unwrap().hot_until, None);
    }
