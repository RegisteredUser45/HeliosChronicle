...
## ClaimFeed execution (B)

Ai `OrderIntent::ClaimFeed` with a system target calls `execute_claim_feed_intent` → B `sky::claim_system` (sets `claimed`, ending wilderness immortality) then H `sensors::sense_system` so the empire gains fog on the claimed system. Parallel to Ai Evacuate → D and Ai ExpandSurvey → H. `salt_emit_enabled` stays default **false**.
